...
(function (ns) {
    ...
    var Solver = Class.extend({
        ...
        _calcTangentVector: function (normal, tangent1, tangent2) {
            ...
            var tan2 = vec3.normalize(vec3.cross(tan1, normal));

            vec3.copy(tan1, tangent1);
            vec3.copy(tan2, tangent2);
        },
        ...
        _solveConstraints: function () {
            ...
            for (var i = 0, l = this._pairs.length; i < l; i++) {
                ...
                for (var j = 0, k = pair.contacts.length; j < k; j++) {
                    ...
                    var velocityA = vec3.add(vec3(bodyA.velocity, 0), vec3.cross(vec3(0, 0, bodyA.angularVelocity * timeStep), r1));
                    var velocityB = vec3.add(vec3(bodyB.velocity, 0), vec3.cross(vec3(0, 0, bodyB.angularVelocity * timeStep), r2));
                    ...
                    //接線ベクトルを求める
                    this._calcTangentVector(normal, tangent1, tangent2);
