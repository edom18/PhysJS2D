...
(function (ns) {
    ...
    var Solver = Class.extend({
        ...
        _calcImpulse: function () {
            ...
            var t = 1 / 16;
            var OA = vec3.cross(r1, vec3(0, 0, objA.angularVelocity * t));
            var VA = vec3.add(vec3(objA.velocity, 0), OA);

            var OB = vec3.cross(r1, vec3(0, 0, objB.angularVelocity * t));
            var VB = vec3.add(vec3(objB.velocity, 0), OB);
            
            // var dV_AB = vec3(vec2.add(objA.velocity, objB.velocity));
            var dV_AB = vec3.sub(VA, VB);
