...
(function (ns) {
    ...
    var Solver = Class.extend({
        ...
        _calcImpulse: function () {
            ...
            var objA = this._objA;
            var objB = this._objB;
            var solverBodyA = new ns.SolverBody(objA);
            var solverBodyB = new ns.SolverBody(objB);
            var bias = ns.contactBias;
            var slop = ns.slop;
            var iteration = ns.iteration;
            var timeStep = 1 / 16;

            pair.friction = sqrt(objA.friction * objB.friction);
            ...
            for (var i = 0, l = this._contacts.length; i < l; i++) {
                ...
                // TODO タイムステップを求める
                var velocityA = vec3.add(vec3(objA.velocity, 0), vec3.cross(vec3(0, 0, objA.angularVelocity * timeStep), r1));

                var velocityB = vec3.add(vec3(objB.velocity, 0), vec3.cross(vec3(0, 0, objB.angularVelocity * timeStep), r2));

                //2つの物体の相対速度を求める（V1 - V2）
                var relativeVelocity  = vec3.sub(velocityA, velocityB);

                var tangent1 = vec3(0.0);
                var tangent2 = vec3(0.0);

                //接線ベクトルを求める
                //calcTangentVector(normal, tangent1, tangent2);
                
                var restitution = this._pairType ? 0.5 * (objA.restitution + objB.restitution) : 0.0;

                // 衝突法線方向の計算
                // {
                //     var axis = normal;
                //     // var denom = vec3.dot(K * axis, axis);
                //     // cp.constraint[0].jacDiagInv = 1.0 / denom;
                //     //rhs = Right Hand Side = 右辺
                //     cp.constraints[0].rhs = -(1.0 + restitution) * vec3.dot(relativeVelocity, axis);
                //     cp.constraints[0].rhs -= (bias * Math.min(0.0, cp.distance + slop) / timeStep);
                //     cp.constraints[0].rhs *= cp.constraint[0].jacDiagInv;
                //     cp.constraints[0].lowerLimit = 0.0;
                //     cp.constraints[0].upperLimit = Number.MAX_VALUE;
                ...
                //     // var denom = vec3.dot(K * axis, axis);
                //     // cp.constraint[0].jacDiagInv = 1.0 / denom;
                //     cp.constraints[1].rhs = -vec3.dot(relativeVelocity, axis);
                //     cp.constraints[1].rhs *= cp.constraint[1].jacDiagInv;
                //     cp.constraints[1].lowerLimit = 0.0;
                //     cp.constraints[1].upperLimit = 0.0;
                ...
                //     // var denom = vec3.dot(K * axis, axis);
                //     // cp.constraint[0].jacDiagInv = 1.0 / denom;
                //     cp.constraints[2].rhs = -vec3.dot(relativeVelocity, axis);
                //     cp.constraints[2].rhs *= cp.constraint[2].jacDiagInv;
                //     cp.constraints[2].lowerLimit = 0.0;
                //     cp.constraints[2].upperLimit = 0.0;
                ...
                //     for (var i = 0, l = this._pars.length; i < l; i++) {
                //         
                //     }
                // }
                ...
                // var posCor = vec2.multiplyScalar(dp, bias);
                //2つの物体の相対速度を求める（V1 - V2）
                // var relativeVelocity  = vec3.add(vec3.sub(velocityA, velocityB), vec3(posCor, 0));
