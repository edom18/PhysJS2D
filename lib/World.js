    ...
    var World = Class.extend({
                ...
                for (var i = 0, l = pairs.length; i < l; i++) {
                    for (var j = 0, k = this.pairs.length; j < k; j++) {
                        if (pairs[i].uuid === this.pairs[j].uuid) {
                            this.pairs[j].type = ns.Pair.Type.KEEP;
        ...
        _solveConstraint: function () {
            ...
            for (var i = 0, l = pairs.length; i < l; i++) {
                ...
                solved = new Phys2D.Solver(pair).calcImpulse();
                pair.objA.velocity = vec2.add(pair.objA.velocity, solved.DVA);
                pair.objA.angularVelocity += solved.DOA;

                pair.objB.velocity = vec2.sub(pair.objB.velocity, solved.DVB);
                pair.objB.angularVelocity -= solved.DOB;
            }
        },
