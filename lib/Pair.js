    ...
    var Pair = Class.extend({
        ...
        findNearestContactPoint: function (newPointA, newPointB, newNormal) {
            ...
            for (var i = 0, l = this.contacts.length; i < l; i++) {
                ...
                if (diffA < minDiff && diffB < minDiff && vec2.dot(newNormal, contact.depthPoint) > 0.99) {
        ...
        addContactPoint: function (cp) {
            ...
            var id = this.findNearestContactPoint(cp.pointA, cp.pointB, cp.depthPoint);

            //既存の点と比較して近い点があった場合は点を更新
            if (id >= 0) {

                // debugger;
                var normal = vec2.normalize(cp.depthPoint);
                var normal2 = vec2.normalize(contacts[id].depthPoint);

                if (Math.abs(vec2.dot(normal, normal2)) > 0.99) {
                	// 同一点を発見、蓄積された情報を引き継ぐ
            ...
            if (contacts.length > ns.contactsLimit) {
                ...
                for (var i = 0, l = contacts.length; i < l; i++) {
                    ...
                    dist = vec2.norm(vec2.sub(cp.contactPoint, contacts[i].contactPoint));
        ...
        refreshContactPoint: function () {
            for (var i = 0, l = this.contacts.length; i < l; i++) {
                if (this.contacts[i].refresh(this.objA.matrix, this.objB.matrix) === null) {
                    debugger;
                }
            }
        }
