(function (ns) {
    ...
    var Contact = Class.extend({
        ...
        _calcPenetrationDepth: function () {
            ...
            //いったん、衝突法線方向に移動し、最接近点を検出
            var normal = vec2.normalize(result);
            var norm = vec2.norm(result) * 1.01;
            normal = vec2.multiplyScalar(normal, norm);
            invNormal = vec2.minus(normal);
            ...
            //移動後、三角形同士の最接近点を求める
            {
                ...
                var verticesB = this._objB.getVertices();
                var _rest = null;
            ...
            //移動した分を元に戻す
            this._objB.translate(invNormal);

            this._drawPoint(_rest, {
                color: 'green'
            });
